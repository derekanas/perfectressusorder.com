  ...
  case "add":
    if(!empty($_POST["quantity"])) {
		...
		if((!empty($_SESSION["cart_item"])) && (!empty($productByCode))) {
			...
			$message = 'SELECT * FROM tblproducthair WHERE itemcode="'.$_GET["code"].'" AND hairtexture="'.$texture.'" AND hairlength="'.$hairlength.'" AND haircolour="'.$escitemcolor.'" AND size="'.$hairsize.'"';
			echo "<script type='text/javascript'>alert('$message');</script>";
		}
...
<div id="container">
    ...
    <div data-role="page">
      ...
      <div data-role="main" class="ui-content content">
        ...
        <img src="https://www.perfectressusorder.com/img/pvh-transconn/color/big/nc.jpg" width="250" height="250">
        <h3><u>NATURAL COLOR<br>#NC</u></h3>
        </center>
        
        <form method="post" id="addtocartform" name="addtocartform" action="natural-color.php?action=add&code=<?php echo "PRVN" ?>" onsubmit="return validateForm()" rel="external" data-ajax="false">
        <div class="ui-grid-a">
              <div class="ui-block-a in-between-spacing">
